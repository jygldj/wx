...
├── config.js            # API 配置：千问/DeepSeek 密钥（zsby 通过 ../config.js 引用，共享）
└── zsby/                # 增删卜易子系统
    ...
    │   └── deepseek.js      # DeepSeek-V4-Pro API 调用模块（替换原智谱 GLM）
    └── README.md            # 本文件
```
...
2. **释卦**（`jiegu.html`）：读取 `currentGua` → 构建 `guaInfo`（世应文本化、`yaoDetail` 含动变、用户信息字段）→ 调用 `callQwen(guaInfo)` / `callDeepseek(guaInfo)`
...
释卦页默认使用 **千问 3.7**；另提供 **小D（deepseek-v4-pro）** 备选（页面标注"小D"，亲切易记）。
两模型的提示词完全一致（野鹤老人口吻 + 核心断卦原则 + 六步框架 + 数据使用规则 + 特别断卦规则），确保解读风格与准确性统一。

## 本次改进（2026-08）

### 改进一/二：固化 AI 提示词
- `api/qwen.js`、`api/deepseek.js` 的 `systemPrompt` 统一为：野鹤老人口吻 + 核心断卦原则 + 回答风格（六步框架、应期提示）+ **数据使用规则**（禁止 AI 自行推演世应/动变/六亲）
...
- `callQwen`/`callDeepseek` 内从 `guaInfo` 提取 `userInfo`/`qiGuaTime` 供提示词使用
...
### 模型替换（2026-08）：智谱 GLM → DeepSeek-V4-Pro
- 新增 `api/deepseek.js`（`callDeepseek`），提示词与千问完全一致
- 删除 `api/glm.js`；`config.js` 原 `CONFIG.glm` 改为 `CONFIG.deepseek`（model: `deepseek-v4-pro`）
- 释卦页模型选择：千问 3.7（默认，不显示"默认"标记）/ 小D（deepseek-v4-pro）
- 加载提示、结果记录中的模型名映射：`qwen → 千问 3.7`、`deepseek → 小D`

## 验证方法

1. **排盘数据保存**：起卦后 F12 控制台执行 `JSON.parse(localStorage.getItem('currentGua'))`，确认含 `shiYao`、`yingYao`、`dongDetail`
2. **AI 提示词数据**：释卦时控制台查看传入 `callQwen`/`callDeepseek` 的 `guaInfo`，确认含 `shiYao`、`yingYao`、`yaoDetail`
...
5. **模型切换**：释卦页可切换"千问 3.7"与"小D"，两者均能正常出解读

## 依赖与存储

- `lunar.js`：农历/干支/节气/旬空计算（约 435KB，位于父目录，与文集共享）
- `config.js`：千问（`qwen3.7-flash`）/ DeepSeek（`deepseek-v4-pro`）API 密钥与端点配置（位于父目录共享）
